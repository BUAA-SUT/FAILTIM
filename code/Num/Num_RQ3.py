...
def getTestcase(mr_list, test_case, num_of_samples):
    ...
    for i in range(num_of_samples):
        ...
        for j in range(len(mr_list)):
            # mr = mr_list[j]
            # mr.setTestCase(test_case)
            # mr.original_ts.setInputOutput("input{}".format(i), "output0_{}".format(i))
            # mr.getFollowInput(j)
            for k in range(len(mr_list)):
                mr = mr_list[k]
                mr.setTestCase(test_case)
                mr.original_ts.setInputOutput("input{}_{}".format(i, j), "output0_{}_{}".format(i, j))
                mr.getFollowInput(k)
